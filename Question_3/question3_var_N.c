...
double faire_regression_lineaires();
...
LinearRegression linear_regression(double *x, double *y, int start, int end) {
    int n = end - start;
...
int main(){
    ...
    int Ns = 1000;  /* nb de particules initial */
    ...
    reinitialiser_fichier("TcVsNs.txt");
    while(Ns<9100){ 
        printf("%d\n", Ns);

        double T = 0.1; /*Température initiale */
        ...
        while(T<5){
            ...
            /* Mettre à jour le tableau des énergies d'équilibre avec la moyenne des 50 dernières valeurs */
        ...
        FILE *file2 = fopen("TcVsNs.txt", "a");
        if (file2 == NULL) {
            perror("Erreur d'ouverture du fichier");
            return 1;
        }
        Tc = faire_regression_lineaires();
...
double faire_regression_lineaires(){
    ...
    FILE *file = fopen("energie_equilibre.txt", "r");
    if (file == NULL) {
    ...
    int i = 0;
    while (fscanf(file, "%lf %lf", &x[i], &y[i]) == 2 && i < 500) {
        i++;
    }
    ...
    LinearRegression reg1 = linear_regression(x, y, 100, 120);
    LinearRegression reg2 = linear_regression(x, y, 300, n);

    double droite1[1000], droite2[1000];

    for (i = 0; i < n; i++) {
        droite1[i] = reg1.a * x[i] + reg1.b;
        droite2[i] = reg2.a * x[i] + reg2.b;
    }

    fclose(file);

    double Tc_x = (reg2.b - reg1.b) / (reg1.a - reg2.a);
    return Tc_x;
}